	...
	public void updateSpecialUserPath(long startVertex, long endVertex, boolean disobedience, long nextItemSelected, boolean finishPath, User currentUser) {
		...
		try {
			...
			*/
			...
			try {
				if (finishPath) {
					System.out.println("IF DEBUG-FALLBACK: Ruta generada desde endVertex=" + endVertex + " para usuario " + currentUser.userID); // Añadido por Nacho Palacio 2025-05-12

					currentPath = pathStrategy.generatePath(endVertex);
				} else {
					System.out.println("ELSE DEBUG-FALLBACK: Ruta generada desde endVertex=" + endVertex + " para usuario " + currentUser.userID); // Añadido por Nacho Palacio 2025-05-12
