...
public class DrawFloorGraph {
	...
	public mxGraphComponent drawFloor(File roomFile, File itemFile, boolean removeVertexLabel, boolean removeEdges, int numberFloor) {
		...
		double vertexY = 0;
		String icon = null;
		int doorID = dataAccessItemFile.getNumberOfItems() + 1;// 241;
		// For each room:
...
//				}
			...
			for (int it = 1; it <= numberOfItems; it++) {
				...
				if (room.equalsIgnoreCase(roomLabel)) {
					...
					/*
					// Lado 1
					double cx1 = Double.valueOf(array[0].split(", ")[0]).doubleValue();
					double cy1 = Double.valueOf(array[0].split(", ")[1]).doubleValue();
					// Lado 2
					double cx2 = Double.valueOf(array[1].split(", ")[0]).doubleValue();
					double cy2 = Double.valueOf(array[1].split(", ")[1]).doubleValue();
					// Lado 3
					double cx3 = Double.valueOf(array[2].split(", ")[0]).doubleValue();
					double cy3 = Double.valueOf(array[2].split(", ")[1]).doubleValue();
					// Lado 4
					double cx4 = Double.valueOf(array[3].split(", ")[0]).doubleValue();
					double cy4 = Double.valueOf(array[3].split(", ")[1]).doubleValue();

					double dx = WITDH; // / (double) 2;
					double dy = HEIGHT; // / (double) 2;

					if (numberFloor == 4 || numberFloor == 5) {
						dx *= -1;
						dy *= -1;
					}
					
					// 1-2: esta en Y, muevo solo la x
					Line2D line_1_2 = new Line2D.Double(cx1, cy1, cx2, cy2);
					if (line_1_2.ptSegDist(vertexY, vertexX) >= 0 && line_1_2.ptSegDist(vertexY, vertexX) < 10) {
						vertexY += dx;
					}
					// 2-3: esta en X, muevo solo la y
					Line2D line_2_3 = new Line2D.Double(cx2, cy2, cx3, cy3);
					if (line_2_3.ptSegDist(vertexY, vertexX) >= 0 && line_2_3.ptSegDist(vertexY, vertexX) < 10) {
						vertexX -= dy;
					}
					// 3-4: esta en Y, muevo solo la x
					Line2D line_3_4 = new Line2D.Double(cx3, cy3, cx4, cy4);
					if (line_3_4.ptSegDist(vertexY, vertexX) >= 0 && line_3_4.ptSegDist(vertexY, vertexX) < 10) {
						vertexY -= dx;
					}
					// 4-1: esta en X, muevo solo la y
					Line2D line_4_1 = new Line2D.Double(cx4, cy4, cx1, cy1);
					if (line_4_1.ptSegDist(vertexY, vertexX) >= 0 && line_4_1.ptSegDist(vertexY, vertexX) < 10) {
						vertexX += dy;
					}
					*/
...
//				vertex = (mxCell) graph.insertVertex(parent, null, vertexLabel, vertexY, vertexX, WITDH, HEIGHT, icon+";noLabel=1;opacity=0");
	...
	private void addCornerRelated(int room, DataAccessRoomFile dataAccessRoomFile, mxCell[] vertices, boolean removeEdges, mxGraph graph, Object parent, int numInvDoors) {
		String roomLabel = dataAccessRoomFile.getRoomLabel(room);
