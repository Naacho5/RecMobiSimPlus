...
import es.unizar.recommendation.UserStats;
...
import es.unizar.recommendation.contextaware.KGCovidAwareRecommendation;
...
public class Simulation {
	...
	// Cuánto tiempo representan en la realidad -> Velocidad de la simulación
	private double timeForIteration; // =1; *In order to go faster: =30 seconds or =2 seconds ("screenRefreshTime"=0.5 seconds)
	// Cada cuántos segundos se itera -> Duración de la iteración en simulación
	...
	//	API Connection with the Recommender module (only KG-Infection model)
	private RecommenderAPIConnector kgInfectionRecommenderConnector;
	private String URL_KGInfectionRecommenderConnector = "http://localhost:5000";
	
	// Experiment
	private int numberOfSpecialUsers; // =1;
	private int numberOfNonSpecialUsers; // =175;
	...
	public Simulation(int timeAvailableUser, int delayObservingPainting, double timeForIteration, double screenRefreshTime, double timeForThePaths, double userVelocity, double kmToPixel, int ttl,
			int timeOnStairs, int minimumTimeToUpdateRecommendation, int communicationRange, int maxKnowledgeBaseSize, int communicationBandwidth, int latencyOfTransmission, int numberOfSpecialUsers,
			int numberOfNonSpecialUsers, String nonSpecialUserPaths, String pathStrategy, String recommendationAlgorithm, float thresholdRecommendation, int howMany, String propagationStrategy,
		...
		this.kgInfectionRecommenderConnector = new RecommenderAPIConnector();
		
		// Creates the connection to the KG-Infection recommender module (REST API)
//		System.out.println("************** RecType --> " + recommendationAlgorithm);
		if (recommendationAlgorithm.equalsIgnoreCase("KG-Infection (KG-Infection)")) {
			try {
//				this.kgInfectionRecommenderConnector = new RecommenderAPIConnector();
				MainSimulator.printConsole("API Connection succedeed", Level.WARNING);
				
			}
			catch(Exception e) {
				System.err.println("Error connecting to the KG-Infection module");
			}
		}
		...
		// Experiment:
		this.numberOfSpecialUsers = numberOfSpecialUsers;
		this.numberOfNonSpecialUsers = numberOfNonSpecialUsers;
		...
		// Auxiliary parameters:
		this.numberOfUser = numberOfSpecialUsers + numberOfNonSpecialUsers;
		this.userList = new ArrayList<User>(numberOfUser);
		this.ended = new ArrayList<User>();
		// Non-RS users:
		boolean isSpecialUser = false;
		for (int i = 0; i < numberOfNonSpecialUsers; i++) {
			int userID = i + 1;
			...
			try {
				...
				System.out.println("***+++*** Exception al crear usuarios");
				log.log(Level.SEVERE, e.toString());
				log.log(Level.SEVERE, "Special/non-RS user image files not correct");
			}
			
			// Let's create the user in the Recommender module
			String response = kgInfectionRecommenderConnector.callGETRecommenderAPI(this.URL_KGInfectionRecommenderConnector + "/user");
			MainSimulator.printConsole("User created in KG-Infection Recommender: USER " + userID, Level.WARNING);
			
		}
		// RS users:
		isSpecialUser = true;
		int specialUser = numberOfNonSpecialUsers;
		for (int i = numberOfNonSpecialUsers; i < numberOfUser; i++) {
			specialUser += 1;
			...
			try {
				...
				// Let's create the user in the Recommender module
				String response = kgInfectionRecommenderConnector.callGETRecommenderAPI(this.URL_KGInfectionRecommenderConnector + "/user");
				MainSimulator.printConsole("Spacial user created in KG-Infection Recommender: USER " + specialUser, Level.WARNING);
			} catch (IOException e) {
				log.log(Level.SEVERE, e.toString());
				log.log(Level.SEVERE, "Special/non-RS user image files not correct");
			}
	...
	public void initializeMoodOfUsers() {
		int[] moodValues = { 10, 11, 12 }; // XXX Change to constant values!!!
...
//				long startVertex = dataAccessGraphFile.getRandomDoor();
				Pair<Integer, Long> p = dataAccessGraphFile.getRandomItemAndRoom();
				currentUser.room = Math.toIntExact(p.getF());
				long startVertex = p.getS();
				
//				initialRoom = -5;
//				for (int i = )
//				for (int num_doors = Integer.valueOf())
//				
...
//				System.out.println("+++++++++ Initial room for special user ++++++++\n" + startVertex + "\n" + this.getRoom(startVertex));
				// The RS user path is updated with the hybrid recommendation algorithm.
				//System.out.println(currentUser.userID);
//				currentUser.room = this.getRoom(startVertex); // rhermoso: Initialises special users' initial room
...
//			System.out.println("locationNextIteration\n " + locationNextIteration); 
		}
	}
...
//		System.out.println("***** Beginning of Generate Non-Special User Paths");
...
//		System.out.println("***** Tries to UPDATE *****");
		
//		System.out.println("*** Start vertex in room: " + this.getRoom(startVertex));
//		System.out.println("*** End vertex in room: " + this.getRoom(endVertex));
		...
		try {
			...
			if (recommendationType.equalsIgnoreCase("Completely-random (FULLY-RAND)")) {
				...
				postfiltering.recommendBaseline(recommendedItems);	// NoSuchUserException -> SOLUCIONADO (Check de límites en función recommend
			...
			} else if (recommendationType.equalsIgnoreCase("Exhaustive visit (ALL)")) {
			...
			else if (recommendationType.equalsIgnoreCase("KG-Infection (KG-Infection)")) {
				KGCovidAwareRecommendation recommender = new KGCovidAwareRecommendation(dataModelSpecialUser, dataAccesLayerDBMuseum, 0.1);
//				long currentContext = getCurrentContext(currentUser);
				
				List<RecommendedItem> candidateItemsFromRecommender = recommender.recommend(currentUser.userID, currentUser.room+400, this.getOccupancies(), getHowMany(), currentUser.getItemsVisited());
				
//				System.out.println("**** CandidateItems *****");
//				System.out.println(candidateItemsFromRecommender);
				
				recommendedItems = this.graphSpecialUser.transformItemIDs(candidateItemsFromRecommender,
																			currentUser.getItemsVisited());
				
				System.out.println("*** Recommended items: " + recommendedItems);
				postfiltering.recommendBaseline(recommendedItems); // NoSuchUserException
				// The path is obtained from the recommended items.
				currentPath = postfiltering.getFinalPath();
				System.out.println("Path from the RecModel API service -> " + currentPath);
				//					recommendedItems = postfiltering.recommendIdeal(candidateItemsFromRecommender);
				...
				log.log(Level.SEVERE, "Finished: KG-Infection");
			}
			else {
...
//			userStats.addTime(timeForIteration);
...
//					long v1 = Long.valueOf(array[0]).longValue();
					...
					visit = (v2 <= this.numberOfITems) ? 1 : 0; 
				}
				
				// XXX TO TEST
				if (currentUser.room > 0) {
//					System.out.println("Occupancies --> " + this.getOccupancies());
					userStats.addRisk(Double.valueOf(this.kgInfectionRecommenderConnector.riskInRoom(currentUser.room, this.getOccupancies(), visit)));
				}
	...
	public synchronized void updateUsers(Map<Integer,UserInfo.UserState> stateOfUsers,Map<Pair<Integer,Integer>,Double> timeUsersInRooms) {
		...
		initialTimeTotal = System.currentTimeMillis();
//		System.out.println("Updating user positions: ");
		MainSimulator.printConsole("Updating user positions: ", Level.INFO); //XXX
...
//			Long moveTime = System.currentTimeMillis();
			...
			while ((availableTimeOfUsers[userPosition] > 0) && (currentTimeOfUsers[userPosition] < getTimeAvailableUserInSecond())) {
				int previousRoomOfUser = currentUser.room; //XXX Change access to method call?
				currentUser.getRoomOfTheUser();
//				System.out.println("[" + u.userID + "] is in room " + currentUser.room);
...
//				}
				...
				// The current path.
				path = graphSpecialUser.paths.get(userPosition); // XXX Move inside the if statement below (only for special users).
				
//				System.out.println("[" + u.userID + "]Gets path for special user");
					...
					if (currentUser.isSpecialUser) {						
						...
						if (v1 <= this.numberOfITems && v2 > this.numberOfITems) {
							...
							if (!itemUpdated.contains(v1)) {
								...
								System.out.println("----- User " + currentUser.userID + "----> UPDATES - PATH NOT FINISHED L. 1009, v1 = " + v1 + " - v2 = " + v2);
								...
								log.log(Level.INFO, "      Tiempo en actualizar ruta (usuario especial tiene más cosas que ver en la sala): " + (finalTime - initialTime));
							}
						}
					}
...
//					System.out.println("[" + u.userID + "] Moving from " + v1 + " to " + v2); 

					// If it is an item (not a door).
					if (v2 <= this.numberOfITems) {
//						System.out.println("###### user position -> " + availableTimeOfUsers[userPosition]);
//						System.out.println("###### voting[userPosition] -> " + voting[userPosition]);
						...
						if ((availableTimeOfUsers[userPosition] >= 0) && (voting[userPosition] == true)) {
							...
							// Not sure if this is the place to add the visitedItem
							if (!(currentUser.getItemsVisited().contains(item))) {
								currentUser.addVisitedItem(item);
							}
...
//							System.out.println("[" + u.userID + "] Generates rating");
...
//							ui.item = v2;
							...
							if (!isChangedItemByRecommender) {
								initialTime = System.currentTimeMillis();
//								System.out.println("[" + u.userID + "] Not changed by recommendation");
...
//								System.out.println("[" + u.userID + "] Observing item " + v2);
								ui.action = "Observing item";
								ui.item = v2;
							}
...
//							System.out.println("[" + u.userID + "] Moving to other item " + userPositionInPath[userPosition]);
							...
							if (v2 > this.numberOfITems) {
//								System.out.println("[" + u.userID + "] v2 is a door or stairs" + v2);
...
//							moveTime = finalTimeMovement - initialTimeMovement;
							...
							if (connectedStairs) {
//								System.out.println("[" + u.userID + "] Connected stairs ");
...
//							System.out.println("[" + u.userID + "] moves locationNextIteration[userPosition-> " + userPosition +"] Room->" + room); 
							finalTimeMovement = System.currentTimeMillis();
...
//								ui.item = null;
							...
							log.log(Level.FINE, "      - FUNCIÓN MOVE: " + (finalTimeMovement - initialTimeMovement));
						}
...
//						moveTime = finalTime - initialTime;
					...
					initialTime = System.currentTimeMillis();
					log.log(Level.INFO, "   Path SÍ acabado");
					...
					if (currentUser.isSpecialUser) {
						try {
							...
							// The RS user path is updated with the recommendation algorithm.
							System.out.println("----> UPDATES - PATH FINISHED L. 1283");
...
//				}
	...
	public float generateRating(long uID, long itemID, long contextID) {
		...
		try {
			...
			if (dataModelMuseumDB.getItemIDsFromUser(uID).contains(itemID)) {
				...
				log.log(Level.INFO, "Valoración: " + (ifCheck - init) + ";" + (getRating - ifCheck) + ";" + (print - getRating) + "; Total: " + ((ifCheck - init) + (getRating - ifCheck) + (print - getRating)));
			} else {
	...
	public void specialUserDisobedience(User currentUser) {
		...
		if ((factorDisobedience <= Configuration.simulation.getProbabilityUserDisobedience())) {
			...
			System.out.println("----> UPDATES - DISOBEDICENCE L. 1587");
		...
		for (int userPosition = 0; userPosition < Configuration.simulation.userList.size(); userPosition++) {
			...
			if (currentUser.isSpecialUser) {
				...
				if ((numberOfItems >= Configuration.simulation.getNumberVoteReceived()) && (currentTimeOfUsers[currentUser.userID - 1] < Configuration.simulation.getTimeAvailableUserInSecond())
					...
					long endVertex = Long.valueOf(cleanEdge(edge)[1]).longValue();
					// The RS user path is updated with the recommendation algorithm.
					System.out.println("----> UPDATES - PATH RECOMMENDER L. 1826");
					updateSpecialUserPath(startVertex, endVertex, false, 0, false, currentUser);
				}
			}
		}
	...
	/**
	 * Calculates de occupancy in a room 
	 * @param room Room ID
	 * @return
	 */
	public int getOccupancy(int room) {
		int occupancy = 0;
		for (User u: userList) {
			if (u.room == room) {
				occupancy++;
	...
	/**
	 * Returns a map with occupancies per room
	 * @return A map with occupancies
	 */
	public Map<String, Integer> getOccupancies() {
		...
		for(int room = 1; room <= numberOfRooms; room++) {
			mapToReturn.put(String.valueOf(room), this.getOccupancy(room));
		}
	...
	public void updateUserDistances(int timeStamp) {
		...
		if (timeStamp < getTimeAvailableUserInSecond()) {
			...
			// For every RS user
			for (int i = numberOfNonSpecialUsers; i < userList.size(); i++) {
				User special = userList.get(i);
				for (int j = 0; j < numberOfNonSpecialUsers; j++) {
...
//				System.out.println("predictedRatings -> " + predictedRatings);
	...
	public int getNumberOfSpecialUser() {
		return numberOfSpecialUsers;
	}

	public int getNumberOfNonSpecialUser() {
		return numberOfNonSpecialUsers;
	}
