...
//			System.out.println("SVG");
	...
	/**
	 * Returns a random item ID, to initialize a random position for the special user
	 * @return
	 */
	public Pair<Integer, Long> getRandomItemAndRoom() {
		Random rand = new Random();
		int randomRoom = 0;
		int nTotalItems = 0;
		while (nTotalItems == 0) {
			randomRoom = rand.nextInt(getNumberOfRoom()) + 1;
			nTotalItems = this.getNumberOfItemsByRoom(randomRoom);
		}
		
//		System.out.println("Random room -> " + randomRoom + " - ntotalItems -> " + nTotalItems);
		return new Pair<Integer, Long>(randomRoom, this.getItemOfRoom(rand.nextInt(nTotalItems) + 1, randomRoom));
	}
